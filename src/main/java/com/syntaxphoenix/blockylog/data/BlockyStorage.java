...
public class BlockyStorage {
    ...
    public static final long MAX_USAGE = 20000L;
    ...
    private long timestamp0, timestamp1 = 0;
    ...
    public Connection connection() {
        ...
        try {
            if ((timestamp0 - timestamp1) < MAX_USAGE || (connection == null || connection.isClosed() || connection.isReadOnly())) {
                connect();
            }
    ...
    public Future<?> loadDataFromFile(int x, int y, int z) {
        return service.submit(() -> {
            ...
            try {
                PreparedStatement prepare = connection().prepareStatement("SELECT * FROM PlayerBlocks WHERE x = ? AND y = ? AND z = ?");
                prepare.setInt(1, x);
                prepare.setInt(2, y);
                prepare.setInt(3, z);
