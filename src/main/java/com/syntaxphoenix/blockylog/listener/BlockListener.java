...
public class BlockListener implements Listener {
    ...
    public void onGrow(StructureGrowEvent event) {
        if (event.getSpecies() == null || event.isCancelled()) {
            return;
        }
        BlockyStorage storage = api.getStorage(event.getWorld());
        ...
        for (BlockState state : states) {
            ...
            System.out.println(data.getPlayerId().toString());
    ...
    public void onPlace(BlockPlaceEvent event) {
        ...
        Location loc = event.getBlock().getLocation();
        api.getStorage(loc.getWorld())
            .setData(new BlockyData(loc.getBlockX(), loc.getBlockY(), loc.getBlockZ(), event.getPlayer().getUniqueId()));
    }
